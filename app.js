...
const authirName = document.querySelector(".name");
...
window.addEventListener("scroll", () => {
  ...
  saluteTextContainer.style.transform = `translateY(-${offset*0.3}px)`;
  saluteImgContainer.style.transform = `translate(${offset*0.1}px,${offset*0.7}px)`;
  authirName.style.transform = `translateX(${offset * 0.03}px)`;
  jobTitleContainers[0].style.backgroundPositionY = `${offset*0.5}px`;
  jobTitleContainers[1].style.backgroundPositionY = `-${offset*0.5}px`;
  jobTitles[0].style.transform = `translateX(calc(200vh - ${offset * 0.9}px))`;
  ...
  phones[1].style.transform = `translateX(calc(700vh - ${offset}px))`
});
